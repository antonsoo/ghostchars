...
var IDENTIFIER_RE = /[\p{L}\p{M}\p{Nd}\p{Pc}][\p{L}\p{M}\p{Nd}\p{Pc}‌‍]*/gu;
...
function scanConfusables(text) {
  ...
  for (const token of tokens) {
    ...
    if (scripts.size > 1) {
      const skeleton2 = skeletonOf(token.text);
      findings.push({
        rule: "confusable",
        severity: "warning",
        ...
        message: `Identifier "${token.text}" mixes scripts (${[...scripts].join(", ")}) in one word -- a common homoglyph-attack shape. Skeleton: "${skeleton2}".`,
...
function reveal(text) {
  ...
  while (i < text.length) {
    ...
    if (finding2) {
      const label = finding2.codePoints.map((cp2) => `U+${cp2.toString(16).toUpperCase().padStart(4, "0")} ${ABBR.get(cp2) ?? shortName(finding2, cp2)}`).join(" ");
      out += `\u27E6${label}\u27E7`;
